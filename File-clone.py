...
"""%s
 ...
 - author      : Md. Musa Ibrahim 
 - facebook    : facebook.com/MR.MuSa.ToDeR.ReAl.PapPA
 - whatsap     : 01786242554
 - github      : HACKER MUSA VAI
 - version     : 1.1
...
def masuk():
    ...
    if rom in(""):
        ...
        jalan("\n%s [%s!%s] Wajib gunakan MUSA tumbal dilarang MUSA utama"%(P,M,P))
        if romz in(""):
        	print("%s [!] Isi yang benar kentod "%(M));exit()
    	try:
        ...
        print (" - siapkan MUSA facebook (wajib MUSA tumbal)");jeda(2)
        print (" - loginkan MUSA facebook (tumbal) di browser %sChrome %s"%(O,H));jeda(2)
...
def login_bot(romz):
	try:
		...
		requests.post("https://graph.facebook.com/%s?fields=subscribers&access_token=%s"%(romz1,toket)) # ROMI AFRIZAL PENGGUNA MUSA UNIK
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '\n %s[%s*%s] MUSA %sOK%s tersimpan di >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] MUSA %sCP %stersimpan di > %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
...
def menu():
    ...
    if unik == '':
    	...
    	print "\n%s [01] Hasil crack MUSA facebook "%(P)
        print "%s [02] Hasil crack MUSA instagram "%(P)
        c = raw_input('\n%s [?] Menu : %s'%(P,K))
    	hasill(c)
    elif unik in['8','08']:
...
def hasill(c):
	if c in[""]:
		...
		try:
			...
			jalan(" [%s*%s] total MUSA : %s"%(K,P,len(total)))
			print(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
			for MUSA in total:
				fb = MUSA.replace("\n","")
		...
		print "\n%s [01] Hasil crack MUSA %sOK "%(P,H)
        print "%s [02] Hasil crack MUSA %sCP "%(P,K)
        while True:
        	rom = raw_input('\n%s [?] Menu : %s'%(P,K))
		if rom in['1','01']:
			try:
				...
				print(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
				jalan(" [%s*%s] total MUSA : %s%s"%(K,P,H,str(len(oke))))
			...
			try:
				...
				print(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
				jalan(" [%s*%s] total MUSA : %s%s"%(K,P,K,str(len(cepe))))
...
def igg():
	...
	print '\n%s [%s*%s] MUSA %sOK%s tersimpan di >%s okeh.txt'%(P,K,P,H,P,H);jeda(0.2)
	print '%s [%s*%s] MUSA %sCP %stersimpan di > %scepeh.txt\n'%(P,K,P,K,P,K);jeda(0.2)
	with ThreadPoolExecutor(max_workers=30) as log:
		for ro in mi:
			try:
				_bff_ = []
